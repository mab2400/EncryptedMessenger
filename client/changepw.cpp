...
int main(int argc, char **argv)
{
	if (argc != 5)
	{
	    std::cerr << "usage: " << argv[0] << " <hostname> <username> <password> <new_password>" << std::endl;
	    exit(1);
	}
	...
	if (pid < 0) 
	{
	    ...
	    // The shell script creates the CSR file, called <username>.csr.pem
	...
	fclose(f);
	remove_file(csr_name); // No need for it anymore
