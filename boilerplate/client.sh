...
" >> $HOME/rootca/intermediate/client.cnf

 # Generating a Private Key
 cd $HOME/rootca
 openssl genrsa -aes256 \
                -out $HOME/rootca/intermediate/private/client.key.pem 2048
 chmod 400 $HOME/rootca/intermediate/private/client.key.pem

 # Using the private key to create a certificate signing request (CSR).
 openssl req -config $HOME/rootca/intermediate/client.cnf \
             -key $HOME/rootca/intermediate/private/client.key.pem \
             -new -sha256 -out $HOME/rootca/intermediate/csr/client.csr.pem

 # Creating the Certificate based on that CSR.
 openssl ca -config $HOME/rootca/intermediate/client.cnf \
            -extensions usr_cert -days 375 -notext -md sha256 \
            -in $HOME/rootca/intermediate/csr/client.csr.pem \
            -out $HOME/rootca/intermediate/certs/client.cert.pem
 chmod 444 $HOME/rootca/intermediate/certs/client.cert.pem

 # Verify the certificate
 openssl x509 -noout -text \
              -in $HOME/rootca/intermediate/certs/client.cert.pem
